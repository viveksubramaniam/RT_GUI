...
while i<=1800:
    ...
    def days_after_J2000():
        ...
        JD = 367.0*year-int(7.0*(year+int((month+9.0)/12.0))/4.0)+int(275.0*month/9.0)+day+UT/24.0+1721013.5- 0.5*extra/math.fabs(extra)+0.5+0.229086
        return JD
    def LST(longitude):
        year=time.gmtime()[0]
        month=time.gmtime()[1]
        ...
        jd = b + c + d - 730550.5 + day + (hour + minute/60.0 + second/3600.0)/24.0                                  #Days since J2000
        jt = jd/36525.0                                                                                              #julian centuries since J2000.0
        mst = 280.46061837 + 360.98564736629*jd + 0.000387933*jt*jt - jt*jt*jt/38710000 + math.degrees(longitude)
        if (mst > 0):
            while (mst > 360.0):
                mst = mst - 360.0
        else:

...
# images
bits_logo = pygame.image.load(r"Project RT\bits_2.jpg")
ncra_logo = pygame.image.load(r"Project RT\NCRA.png")
rt_logo = pygame.image.load(r"Project RT\rt_2.png")
bim = pygame.image.load(r"Project RT\Space.jpg")
black_image = pygame.image.load(r"Project RT\black.png")
black_spcae = pygame.image.load(r"Project RT\black_space.jpg")
...
def main_menu():
    ...
    while menu:
        ...
        screen.fill(white)
        screen.fill(black, (0.4*screen_width, 0, screen.get_width(), screen.get_height()))
        ...
        for event in pygame.event.get(): 
            ...
            if (event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN) or event.type == pygame.MOUSEBUTTONDOWN:
                if selected =="ATP":
                    webbrowser.open('https://projectrtbits.wordpress.com/', new=2)
                elif selected == "RAT":
                    RAT() 
        ...
        if pygame.Rect((0.2*screen_width - int(track_rect[2] / 2)), 0.3125*screen_height, track_rect[2], track_rect[3]).collidepoint(pos) and pressed or selected == "Track":
            track_text = text_format("Track", 40, black)
            pygame.draw.rect(screen, black, ((0.2*screen_width - int(track_rect[2] / 2)), int(0.3125*screen_height), track_rect[2], track_rect[3]), 2)
            selected = "Track"
        if pygame.Rect((200 - int(map_rect[2] / 2)), 330, map_rect[2], map_rect[3]).collidepoint(pos) and pressed or selected == "Map":
            map_text = text_format("Map", 40, black)
            pygame.draw.rect(screen, black, ((195 - int(map_rect[2] / 2)), 330, map_rect[2] + 10, map_rect[3]), 2)
            selected = "Map"
        if pygame.Rect((200-int(RAT_rect[2] /2)), 410, RAT_rect[2], RAT_rect[3]).collidepoint(pos) and pressed or selected == "RAT":
            RAT_text = text_format("RA-Dec to ALT-Az", 40, black)
            pygame.draw.rect(screen, black, ((195-int(RAT_rect[2] /2)), 410, RAT_rect[2] + 10, RAT_rect[3]), 2)
            selected = "RAT"
        if pygame.Rect((200-int(history_rect[2]/2)), 490, history_rect[2], history_rect[3]).collidepoint(pos) and pressed or selected == "History":
            history_text = text_format("History", 40, black)
            pygame.draw.rect(screen, black, ((195-int(history_rect[2]/2)), 490, history_rect[2] + 10, history_rect[3]), 2)
            selected = "History"
        if pygame.Rect((200 - int(ATP_rect[2] / 2)), 570, ATP_rect[2], ATP_rect[3]).collidepoint(pos) and pressed or selected == "ATP":
            ATP_text = text_format("About the Project", 40,  black)
            pygame.draw.rect(screen, black, ((195 - int(ATP_rect[2] / 2)), 570, ATP_rect[2] + 10, ATP_rect[3]), 2)
            selected = "ATP"




        screen.blit(welcome_message, ((700-int(welcome_message_rect[2]/2)), int(screen_height/4)))
...
def RAT():
    ...
    long_text = text_format("Enter the current Longitude(in degrees):", 40, white)
    long_rect = long_text.get_rect()
    io_rect = pygame.Rect((int(screen_width/4 + lat_rect[2]/2), int(screen_height/8 +100), 200, lat_rect[3]))
    ...
    while RAT:
        ...
        lat_rect_io = pygame.Rect((int(screen_width/4 + lat_rect[2]/2 +5), int(screen_height/8 +100), 200, lat_rect[3]))
        long_rect_io = pygame.Rect((int(screen_width/4 - lat_rect[2]/2 + long_rect[2] +5), int(screen_height/8 +200), 200, long_rect[3]))
        convert_rect = pygame.Rect((int(screen_width/4 - convert_text_rect[2]/2), int(screen_height/2 +200), int(convert_text_rect[2]+10), int(convert_text_rect[3]+10)))
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if event == 1:
                        ...
                        RAD.convert()                                                                #UNDO THIS COMMENT TO EXECUTE CONVERT ON PRESSING ENTER 
                    cfg.text_long = ''
                    cfg.text_lat = ''
                elif event.key == pygame.K_BACKSPACE:
                    if active == 1:
                        cfg.text_lat= cfg.text_lat[:-1]
                    else:
                        cfg.text_long = cfg.text_long[:-1]
                elif event.key == pygame.K_ESCAPE:
                    main_menu()
                else :
                    if active == 1:
                        cfg.text_lat+= event.unicode 
                    else:
                        cfg.text_long+= event.unicode
    
        if active == 1:
            text_lat_render = text_format(cfg.text_lat, 30, white)
        elif active == 2:
            text_long_render = text_format(cfg.text_long, 30, white)
        ...
        screen.blit(text_RAT, (int(screen_width/2 - text_RAT_rect[2]/2), int(screen_height/8)),)
        screen.blit(lat_text, (int(screen_width/4 - lat_rect[2]/2), int(screen_height/8 + 100)))
        screen.blit(long_text, (int(screen_width/4 - lat_rect[2]/2), int(screen_height/8 + 200)))
        ...
        file.write(cfg.text_lat+" "+cfg.text_long+'\n')
        file.close()
